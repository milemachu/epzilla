		...
		if (event instanceof ProcessStatusChangedEvent) {
			...
			if (EpzillaProcess.getInstance().getStatus().equalsIgnoreCase(
				...
				while (iterator.hasNext()) {
					...
					try {
						...
						System.out.println("Non eader node "+listner.getData()+"does not accept the PULSEmessage from rhe Leader");
						continue;
					}
				}
				...
				timer.cancel();
				timer = new Timer();
				timer.schedule(new EpzillaPulseSendTimer(), 60000);
			...
			try {
				if (EpzillaProcess.getInstance().getStatus().equalsIgnoreCase(
					...
					// timer.cancel();
					// timer.schedule(new EpzillaPulseReceiveTimer(), 120000);
				...
				if (e instanceof ConnectException
					...
					try {
						...
						System.out.println("UID message sent to the next node");
						
					} catch (Exception e2) {
					System.out.println("Sending UID to next node is failed. Trying sending UID to any node.");

					for (InetAddress nextIp :ep.getClusterIpList()) {
						try {
							if(!nextIp.getHostAddress().equalsIgnoreCase(InetAddress.getLocalHost().getHostAddress()))
							try {
								LeaderMessageClient
										.sendUidMessage(nextIp.getHostAddress());
								isMessageDelivered=true;
								System.out.println("UID successfully sent to :"+nextIp.getHostAddress());
							} catch (Exception e1) {
					...
					if(!isMessageDelivered){
						System.out.println("Sending UID to any alive node failed. Sending UID to localhost.");
						try {
