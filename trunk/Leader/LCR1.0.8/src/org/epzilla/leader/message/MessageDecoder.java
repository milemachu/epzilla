...
public class MessageDecoder {
	...
	public void decodeMessage(String message) throws UnknownHostException {
		...
		if (Integer.parseInt(strItems[0]) == MessageMeta.LEADER) {
			...
			if (strItems[1].equalsIgnoreCase(
							InetAddress.getLocalHost().getHostAddress())) {
				
				System.out.println("Leader is the Localhost");
				// No need to forward the LEADER message. Do nothing now about that message.				
				//Set the timer for Pulse
				EpzillaProcess.getInstance().fireEpzillaEvent(new PulseTimeoutEvent());
			} else {
				...
				EpzillaProcess.getInstance().fireEpzillaEvent(new ProcessStatusChangedEvent());
				System.out.println("Leader is else and this is  a NON leader. Status changed event fired.");
				
				

				// Forward the Leader message to next host and register this node as a listener.
				try {

					// TODO Remove hard code and get it from the list
					LeaderMessageClient.forwardLeaderElectedMessage(
							"192.168.1.63", message);
					LeaderMessageClient.registerListenerWithLeader(strItems[1], new EpZillaListener());
			...
			System.out.println("PULSE Recieved from the Leader.");
		}
