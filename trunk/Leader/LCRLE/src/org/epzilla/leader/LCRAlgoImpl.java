		...
		if (strArray != null && Integer.parseInt(strArray[1]) >= 1) {
			...
			if (receivedUid == EpzillaProcess.getInstance().getUID()) {
				...
				try {
					...
					try {
						LeaderMessageClient
								.sendLeaderElectedMessage(EpzillaProcess
										.getInstance().getIpNextToThis());
					} catch (Exception e) {
						boolean isMessageDelivered = false;
						for (InetAddress nextIp : EpzillaProcess.getInstance()
								.getClusterIpList()) {
							if (!nextIp.getHostAddress()
								...
								try {
									LeaderMessageClient
											.sendLeaderElectedMessage(nextIp
													.getHostAddress());
						...
						if (!isMessageDelivered) {
							LeaderMessageClient
									.sendLeaderElectedMessage(InetAddress
