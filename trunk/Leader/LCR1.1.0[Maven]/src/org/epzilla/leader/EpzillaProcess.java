...
public class EpzillaProcess {
	...
	public synchronized void setDefaultClusterLeader(InetAddress defaultClusterLeader) {
	...
	public synchronized void setLeaderElectionRunning(boolean isLeaderElectionRunning) {
	...
	public String getIpNextToThis(){
		...
		String nextIp=ep.getClsuterIpWithUid().get(new Integer(EpzillaProcess.UID+1));
		//null if this is the last host
		if(nextIp==null){
			nextIp=new ArrayList<String>(ep.getClsuterIpWithUid().values()).get(0);
		}
			...
			try {
				if (EpzillaProcess.getInstance().getStatus().equalsIgnoreCase(
					...
					**/
					...
					String serverState=LeaderMessageClient.getStateFromRemote(EpzillaProcess.getInstance().getClusterLeader().getHostAddress());
					String serverClusterLeader=LeaderMessageClient.getClusterLeaderFromRemote(EpzillaProcess.getInstance().getClusterLeader().getHostAddress());
					
					if(serverState.equalsIgnoreCase(Status.LEADER.toString())){
						//just send the listener.
						LeaderMessageClient.registerListenerWithLeader(serverClusterLeader, new EpZillaListener());
						//SEnd the PING now. -Redundent check.
						LeaderMessageClient.sendPing(serverClusterLeader);
						//Now I shud deffa get a PULSE. If not this is an error.So wait for it.
					}else if(serverState.equalsIgnoreCase(Status.UNKNOWN.toString()) && serverClusterLeader==null){
...
//					timer.schedule(new EpzillaPulseReceiveTimer(), 360000);
				...
				System.out.println("Server host unavailable at this time. Server is down and initiate a new Leader election.");
				System.out.println(e.getMessage());
				
				if(e instanceof ConnectException || e instanceof java.rmi.ConnectException || e instanceof UnknownHostException){
					...
					try {
						LeaderMessageClient.sendUidMessage(ep.getIpNextToThis());
				...
				if (EpzillaProcess.getInstance().getStatus().equalsIgnoreCase(
					...
					LeaderMessageClient
							.sendRequestNotAccepted(((PingReceivedEvent) event)
									.getData(),RejectReason.NOT_ALLOWED_TO_RECEIVE_PING_NOT_LEADER);
				}
			} catch (MalformedURLException e) {
			...
			if(EpzillaProcess.getInstance().getStatus().equalsIgnoreCase(Status.NON_LEADER.toString())){
				...
				try {
					LeaderMessageClient.sendRequestNotAccepted(((PulseReceivedEvent)event).getData(),RejectReason.NOT_ALLOWED_TO_RECEIVE_PULSE_NOT_NON_LEADER);
				} catch (MalformedURLException e) {
			...
			//Send the pulse to the listener if the status is leader
				...
				try {
					...
					LeaderMessageClient.sendPulse(((ListenerRegisteredEvent)event).getData());
						...
						LeaderMessageClient.sendRequestNotAccepted(((ListenerRegisteredEvent)event).getData(),RejectReason.NOT_ALLOWED_TO_REGISTER_LISTNER_NOT_LEADER);
					}
				} catch (MalformedURLException e) {
