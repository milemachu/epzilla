		...
		if (Integer.parseInt(strItems[0]) == MessageMeta.LEADER) {
			...
			if (strItems[1].equalsIgnoreCase(InetAddress.getLocalHost()
				...
				try {
					...
					LeaderMessageClient.forwardLeaderElectedMessage(EpzillaProcess.getInstance().getIpNextToThis(), message);
					LeaderMessageClient.registerListenerWithLeader(strItems[1],
							new EpZillaListener());
					
					//Set the pulse receive timer here for the first time
					//cannot remove since we register the listener and server supposed to send a pulse for real.
					//But it is possible that 1st pulse also not receiving.
					EpzillaProcess.getInstance().fireEpzillaEvent(new PulseReceivedEvent(strItems[1]));
...
//			EpzillaProcess.getInstance().setLeaderElectionRunning(false);
			...
			System.out.println("Request rejected due to:"+RejectReason.getErrorMessage(Integer.parseInt(strItems[2])));
			
			EpzillaProcess.getInstance().fireEpzillaEvent(new RequestRejectedEvent(Integer.parseInt(strItems[2])));
