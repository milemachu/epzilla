...
//	private static Logger logger = Logger.getLogger(EpzillaNodeInitiator.class);
	...
	private boolean deployRmiInterace() {
		try {
			System.out.println("Deploying RMI Services in:"
	...
	public static void main(String[] args) throws UnknownHostException,
		...
		try {
			...
			System.out.println("Configuration loaded and ready to initialize");
		} catch (IOException e1) {
			System.out.println(("Configuration initialization failed. Exiting system."));
			e1.printStackTrace();
			System.exit(-1);
		}
		...
		if (isServicesDeployed) {
			System.out.println(("RMI Services deployied. Initializing start up sequence."));
			...
			if (isSTMServerRunning) {
				...
				System.out.println(("STM server is running and this node joinging the STM cluster."));
				...
				if (isDefaultLeader) {
					...
					// again. So no problem.
					System.out.println(("No STM is present in the cluster. Launching the leader election in 15 secs since this is the default server."));
					try {
						Thread.sleep(15000);
					...
					//isMessageDelivered==false=> message was not delivered to any one.
					//Wait any try again if it fails again, send UID to localhost
					
					if(!isMessageDelivered){
						System.out.println("UID message not delivered. Trying again.");
					...
					if(!isMessageDelivered){
						...
						LeaderMessageClient.sendUidMessage(InetAddress.getLocalHost().getHostAddress());
						epzillaInstance.setLeaderElectionRunning(true);
						isMessageDelivered=true;
					}
					...
					try {
						...
						System.out.println("Cannot connect to the default node.");
									...
									if (!isLERunning
										...
										continue;
											...
											& status
													.equalsIgnoreCase(Status.UNKNOWN
															.toString())) {
										// LE running and UID received but not the
										// LEADER. So cannot use
										continue;
									} else if (!isLERunning
											& status.equalsIgnoreCase(Status.LEADER
													.toString())) {
										// This is the server.
					...
					if(epzillaInstance.getStatus().equalsIgnoreCase(Status.UNKNOWN.toString()) & !epzillaInstance.isLeaderElectionRunning()){
