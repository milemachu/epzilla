...
public class EpzillaProcess {
	...
	public synchronized void fireEpzillaEvent(IEpzillaEvent event) {		
		...
		if (event instanceof ProcessStatusChangedEvent) {
			...
			//So send a PING to the last known Leader ad check.
			//If the Leader is the same node, it will send a pulse. Else have to get the new leader
			//From another host. No need for a Leader election.
			//TODO Think abt this more before complete.
				try {
					if(EpzillaProcess.getInstance().getStatus().equalsIgnoreCase(Status.NON_LEADER.toString())){
					LeaderMessageClient.sendPing(EpzillaProcess.getInstance().getClusterLeader().getHostAddress());
					}else{
//						EpzillaProcess.getInstance().setClusterLeader(LeaderMessageClient.getClusterLeaderFromRemote(remoteIp))
					}
				} catch (MalformedURLException e) {
