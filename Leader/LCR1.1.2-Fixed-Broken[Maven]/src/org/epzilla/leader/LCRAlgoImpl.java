...
public class LCRAlgoImpl {
	...
	public void runAlgorithm(String message) {
		...
		if (strArray != null && Integer.parseInt(strArray[1]) >= 1) {
			...
			if (receivedUid == EpzillaProcess.getInstance().getUID()) {
				...
				try {
					...
					// remove the hardcode address and get it from the ip
					// list
					try {
						LeaderMessageClient.sendLeaderElectedMessage(EpzillaProcess
								.getInstance().getIpNextToThis());
					} catch (Exception e) {
						boolean isMessageDelivered=false;
						for (InetAddress nextIp : EpzillaProcess.getInstance().getClusterIpList()) {
							if(!nextIp.getHostAddress().equalsIgnoreCase(InetAddress.getLocalHost().getHostAddress())){
								try {
									LeaderMessageClient.sendLeaderElectedMessage(nextIp.getHostAddress());
						...
						if(!isMessageDelivered){
							LeaderMessageClient.sendLeaderElectedMessage(InetAddress.getLocalHost().getHostAddress());
						}
					}
			...
			} else if (receivedUid < EpzillaProcess.getInstance().getUID()) {
